...
startMediasoup()
  .then(() => {
    io.on('connection', (socket) => {
      ...
      socketData[socket.id] = { transportIds: [], producerIds: [], consumerIds: [] };
      ...
      socket.on('cmd-create-team', ({ name }) => {
        ...
        if (users[socket.id] && users[socket.id].teamId) {
          ...
          if (oldTeam) oldTeam.members = oldTeam.members.filter(m => m !== socket.id);
        }
      ...
      socket.on('ptt-signal-send-stop', () => {
        ...
        if (user && user.teamId) {
          socket.to(user.teamId).emit('ptt-signal-stop', { senderSocketId: socket.id });
        }
      ...
      socket.on('transport-produce', async ({ transportId, kind, rtpParameters }, callback) => {
        try {
          ...
          producers[producer.id] = { producerInstance: producer, ownerSocketId: socket.id };
          ...
          const user = users[socket.id];
          if (user && user.teamId) {
            socket.to(user.teamId).emit('new-producer', { producerId: producer.id });
          } else {
      ...
      socket.on('transport-consume', async ({ transportId, producerId, rtpCapabilities }, callback) => {
        try {
          const producerRecord = producers[producerId];
          if (!producerRecord) return callback({ error: 'Target producer context missing' });
      ...
      socket.on('disconnect', () => {
        ...
        teams.forEach(t => t.members = t.members.filter(m => m !== socket.id));
        sendStateUpdate();
    ...
    server.listen(3000,'0.0.0.0', () => console.log('SFU Voice Server running on port 3000'));
  })
  .catch((err) => {
